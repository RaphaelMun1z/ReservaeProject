...
const MOCK_QR = `data:image/svg+xml,${encodeURIComponent(`
	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120">
		...
		<g fill="#090909">
			<path d="M8 8h34v34H8zm7 7v20h20V15zm8 8h4v4h-4zM78 8h34v34H78zm7 7v20h20V15zm8 8h4v4h-4zM8 78h34v34H8zm7 7v20h20V85zm8 8h4v4h-4z"/>
			<path d="M52 8h8v8h-8zm12 0h8v16h-8zM48 28h16v8H48zm8 12h8v8h-8zm12 0h8v20h-8zM44 52h12v8H44zm16 4h8v12h-8zm16-8h8v16h-8zm12 0h20v8H88zM8 52h12v8H8zm16-4h12v16H24zM8 68h28v8H8zm40 4h8v16h-8zm12 0h20v8H60zm24-8h8v20h-8zm12 0h16v8H96zM60 84h12v8H60zm16 4h8v20h-8zm12 0h24v8H88zm-36 12h16v8H52zm36 4h8v8h-8zm12-4h12v12h-12z"/>
		</g>
	</svg>
`)}`;
...
const MOCK_EVENTS = [
	{
		id: "show-natanzinho",
		name: "Natanzinho Lima — São Paulo no 12",
		artistName: "Natanzinho Lima",
		tourName: "Turnê Musical",
		status: "VENDAS ABERTAS",
		startsAt: "2026-06-12T21:00:00-03:00",
		minPrice: 90,
		imageUrl: "https://images.unsplash.com/photo-1501386761578-eac5c94b800a?auto=format&fit=crop&q=85&w=1000",
		venue: { name: "Arena Santos", address: { city: "Santos", state: "SP" } },
	},
	{
		id: "festival-aurora",
		name: "Festival Aurora",
		artistName: "Vários artistas",
		status: "ÚLTIMOS INGRESSOS",
		startsAt: "2026-07-18T18:00:00-03:00",
		minPrice: 140,
		imageUrl: "https://images.unsplash.com/photo-1470229722913-7c092bce28f1?auto=format&fit=crop&q=85&w=1000",
		venue: { name: "Parque Villa-Lobos", address: { city: "São Paulo", state: "SP" } },
	},
	{
		id: "noite-acustica",
		name: "Noite Acústica",
		artistName: "Convidados especiais",
		status: "NOVO",
		startsAt: "2026-08-08T20:30:00-03:00",
		minPrice: 75,
		imageUrl: "https://images.unsplash.com/photo-1507676184212-d0330a15183c?auto=format&fit=crop&q=85&w=1000",
		venue: { name: "Teatro Central", address: { city: "Campinas", state: "SP" } },
	},
];

const MOCK_SECTORS = [
	{ id: "pista", name: "Pista", lotName: "2º lote", price: 90 },
	{ id: "vip", name: "Pista VIP", lotName: "1º lote", price: 180 },
	{ id: "camarote", name: "Camarote", lotName: "Lote único", price: 280 },
];
...
const MOCK_ORDER = {
	id: "mock-order",
	items: [
		{ name: "Pista VIP — Natanzinho Lima", quantity: 2, total: 360 },
	],
	fees: 36,
	total: 396,
};
...
const routeLoaders = {
	"event-list": () => Promise.resolve(MOCK_EVENTS),
	"featured-events": () => Promise.resolve(MOCK_EVENTS),
	"event-detail": () => Promise.resolve(MOCK_EVENTS.find((event) => event.id === readPageParam("eventId")) || MOCK_EVENTS[0]),
	"event-sectors": () => Promise.resolve(MOCK_SECTORS),
	checkout: () => Promise.resolve(normalizeCheckoutDraft(readCheckoutDraft())),
	"order-detail": () => Promise.resolve(MOCK_ORDER),
	"user-tickets": () => Promise.resolve(MOCK_TICKETS),
	"ticket-detail": () => Promise.resolve(MOCK_TICKETS.find((ticket) => ticket.id === readPageParam("ticketId")) || MOCK_TICKETS[0]),
	"user-profile": () => Promise.resolve(MOCK_USER),
	"admin-dashboard": () => Promise.resolve(MOCK_ADMIN),
	"admin-events": () => Promise.resolve(MOCK_EVENTS),
	auth: () => Promise.resolve({ authenticated: true, user: MOCK_USER }),
};

function normalizeCheckoutDraft(draft) {
	if (!draft?.items) return MOCK_ORDER;
	const items = Object.values(draft.items).map((item) => ({
		name: MOCK_SECTORS.find((sector) => sector.id === item.sectorId)?.name || "Ingresso",
		quantity: item.quantity,
		total: item.quantity * item.price,
	}));
	const subtotal = items.reduce((total, item) => total + item.total, 0);
	return { items, fees: subtotal * 0.1, total: subtotal * 1.1 };
}
...
function emptyState(message) {
	return `<p class="mock-empty-state">${escapeHtml(message)}</p>`;
}
...
function showTicketDetailSkeleton() {
	const container = document.querySelector(".mock-ticket-detail");
	if (!container) return;
...
function showOrderSkeleton() {
	const container = document.querySelector(".mock-order-summary");
	if (!container) return;
...
function getErrorTarget(pageKey) {
	...
	if (pageKey === "ticket-detail") {
		return document.querySelector(".mock-ticket-detail") || document.querySelector(".ticket-list");
	}

	if (pageKey === "checkout" || pageKey === "order-detail") {
		return document.querySelector(".mock-order-summary");
	}
...
function renderErrorState(pageKey, error) {
	...
	const message =
		error?.status === 404
			...
			: "Nao foi possivel carregar os dados agora. Confira se os dados locais estao disponiveis e tente novamente.";

	target.innerHTML = `
		<div class="mock-error-state" role="alert">
			<p class="mock-error-title">Nao foi possivel carregar o conteudo</p>
			<p class="mock-error-message">${message}</p>
			<button class="mock-retry-button" type="button" data-mock-retry>Tentar novamente</button>
		</div>
	`;

	target.querySelector("[data-mock-retry]")?.addEventListener("click", () => {
		bootstrapPageData();
	});
}
		...
		if (isTicketGrid) {
			return `
				<div class="ticket-card">
					...
					<div class="tc-divider"></div>
					<div class="tc-action">
						<a class="tc-btn" href="${appRoute(`sector-selection.html?eventId=${encodeURIComponent(event.id || "")}`)}">COMPRAR AGORA</a>
					</div>
				</div>
			`;
		}
		...
		<article class="show-card">
			...
			<div class="sc-content">
				...
				<div class="sc-footer">
					...
					<a class="sc-btn" href="${appRoute(`sector-selection.html?eventId=${encodeURIComponent(event.id || "")}`)}">VER INGRESSOS</a>
				</div>
			</div>
		</article>
	`;
...
function showErrorToast(error) {
	...
	const message =
		error?.status === 404
			...
			: "Nao foi possivel carregar os dados locais. Verifique se os servicos estao online.";
...
async function bootstrapPageData() {
	...
	try {
		...
		console.warn(
			`[MatchPass] Falha ao carregar dados mockados da pagina "${pageKey}".`,
			error,
		);
	}
}
