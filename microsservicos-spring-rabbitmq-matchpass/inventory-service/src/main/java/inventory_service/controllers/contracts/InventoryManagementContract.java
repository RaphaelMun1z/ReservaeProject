...
public interface InventoryManagementContract {
    ...
    ResponseEntity<TicketStatusResponseDTO> checkTicketStatus(@PathVariable String ticketTag);

    @Operation(summary = "Confirmar a venda de um assento bloqueado")
    ResponseEntity<Void> confirmTicketSold(@PathVariable String ticketTag);

    @Operation(summary = "Liberar um assento bloqueado de volta para o inventário")
    ResponseEntity<Void> releaseTicket(@PathVariable String ticketTag);

    @Operation(summary = "Buscar todos os assentos associados a um usuário")
    ResponseEntity<List<TicketStatusResponseDTO>> findUserTickets(@PathVariable String userId);
